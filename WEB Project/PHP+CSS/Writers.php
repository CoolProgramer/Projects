...
$result0=$mysqli->query("Select * from original where id = '104'");
$row0 = $result0->fetch_assoc();
$mainimage = $row0["images"];
$mainheading=$row0["content"];


$result1=$mysqli->query("Select * from original where id = '105'");
$row1 = $result1->fetch_assoc();
$firstimage = $row1["images"];
$firstcontent= $row1["content"];
$firstheading=$row1["name"];

$result2=$mysqli->query("Select * from original where id = '106'");
$row2 = $result2->fetch_assoc();
$moviesheading= $row2["name"];
$movies1=$row2["content"];

$result3=$mysqli->query("Select * from original where id = '107'");
$row3 = $result3->fetch_assoc();
$secondimage = $row3["images"];
$secondcontent= $row3["content"];
$secondheading=$row3["name"];

$result4=$mysqli->query("Select * from original where id = '108'");
$row4 = $result4->fetch_assoc();
$movies2=$row4["content"];

$result5=$mysqli->query("Select * from original where id = '109'");
$row5 = $result5->fetch_assoc();
$thirdimage = $row5["images"];
$thirdcontent= $row5["content"];
$thirdheading=$row5["name"];

$result6=$mysqli->query("Select * from original where id = '110'");
$row6 = $result6->fetch_assoc();
$movies3= $row6["content"];


$result7=$mysqli->query("Select * from original where id = '111'");
$row7 = $result7->fetch_assoc();
$fourthimage = $row7["images"];
$fourthcontent= $row7["content"];
$fourthheading=$row7["name"];

$result8=$mysqli->query("Select * from original where id = '112'");
$row8 = $result8->fetch_assoc();
$movies4=$row8["content"];


$result9=$mysqli->query("Select * from original where id = '113'");
$row9 = $result9->fetch_assoc();
$fifthimage = $row9["images"];
$fifthcontent= $row9["content"];
$fifthheading=$row9["name"];

$result10=$mysqli->query("Select * from original where id = '114'");
...
<div  class="div2">  <?php echo"<img src=$thirdimage  style='width:275px;height:350px'>" ?>
</div>
